...
- **Data linked to you:** None (no account; anonymous device token).
- **Data not linked to you:** Purchases (subscription status); "Other data" — dream
  text sent transiently to the AI provider for interpretation (not stored by you).
...
Mirror the above: no account, transient text sent for processing, purchases handled by
Play/RevenueCat.
...
> of an interpretation. Subscriptions unlock more daily interpretations; test account
> not required (anonymous). Backend is a stateless proxy that does not store user data.

## Screenshots to produce
1. Record screen (mic, "Tell me your dream")
2. A finished interpretation with a quote card
3. Journal list
4. Insights (recurring symbols)
5. Paywall
