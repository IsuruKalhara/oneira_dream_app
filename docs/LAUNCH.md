# Oneira — deploying the backend & making money
### A Bitfuzed product
...
them (`lib/core/config.dart` → `ONEIRA_API_BASE`):
...
`oneira/src/server.mjs` (Node, local) and `oneira/worker/src/index.js`
...
`oneira/data/` currently contains only `samples/`. **The books have never been
...
```bash
npx wrangler vectorize create oneira-kb --dimensions=1536 --metric=cosine
```
...
flutter build apk --release \
  --dart-define=ONEIRA_API_BASE=https://oneira-proxy.<subdomain>.workers.dev \
...
You cannot buy installs at $0.25 in this category. **So Oneira is an organic
...
> **Most dream apps invent an answer. Oneira shows you the page.**
...
`Oneira — Dream Journal` / `Dream diary, grounded in books`.
