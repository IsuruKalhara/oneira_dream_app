# Dreamlore — dream knowledge base + `/explain` API (Phase 0)

_"Dreamlore" is a working name (Greek **ὄνειρα**, "dreams" — the root of Artemidorus' Oneirocritica). Rename freely._
...
| `LLM_PROVIDER` | `stub` | `openai` (production default), `claude` |
| `OPENAI_API_KEY` | — | required for `LLM_PROVIDER=openai` |
| `OPENAI_MODEL` | `gpt-4o-mini` | interpretation model |
| `ANTHROPIC_API_KEY` | — | required for `LLM_PROVIDER=claude` |
| `CLAUDE_MODEL` | `claude-sonnet-5` | any current Claude model |
...
| `src/llm.mjs` | OpenAI + Claude calls (structured output) + offline stub |
| `src/tokenize.mjs` | Tokenizer + sparse TF-IDF embedding. Shared by ingest, server and Worker |
| `src/retrieve.mjs` | Ranking for the bundled KB: alias expansion + specificity penalty |
| `eval/` | 18-dream retrieval benchmark — `npm run eval` |
...
The deployed Worker instead bakes the whole index into its bundle — see
`worker/README.md`.
